...
# class Ks_Rating_avg(models.AbstractModel):
...
#         read_group_res = self.env['rating.rating'].read_group(domain, ['rating:avg'], groupby=['res_id'],
#                                                               lazy=False)  # force average on rating column
#         mapping = {item['res_id']: {'rating_count': item['__count'], 'rating_avg': item['rating']} for item in
#                    read_group_res}
#         for record in self:
#             record.rating_count = mapping.get(record.id, {}).get('rating_count', 0)
#             record.rating_avg = mapping.get(record.id, {}).get('rating_avg', 0)
#             record.write({'ks_rating_avg':record.rating_avg})
#
...
class KSRATING(models.AbstractModel):
    ...
    def _compute_rating_stats(self):
        """ Compute avg and count in one query, as thoses fields will be used together most of the time. """
        domain = self._rating_domain()
        read_group_res = self.env['rating.rating'].read_group(domain + [('rating', '>=', 1)], ['rating:avg'],
                                                              groupby=['res_id'],
                                                              lazy=False)
...
class KSPRODUCTVIDEO(models.Model):
    ...
    product_video = fields.Binary("Video in Mp4")

    @api.depends('product_video')
    def _compute_embed_code(self):
        for image in self:
            id = self.env['ir.attachment'].sudo().search([('res_field', '=', 'product_video'), ('res_id', '=', image.id)]).id
            if id:
                image.embed_code = '<iframe class="embed-responsive-item" src="/web/content/%s" allowFullScreen="true" frameborder="0" autoplay="1" allow="autoplay"></iframe>' % id
            else:
                image.embed_code = id
...
class KSBRANDDESC(models.Model):
    _inherit = 'ks_product_manager.ks_brand'
    _description = 'For Brand description'

    ks_brand_description = fields.Char(string='Description')
