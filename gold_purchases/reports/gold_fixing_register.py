...
class GoldFixingRegister(models.AbstractModel):
    _name = "gold.fixing.register"
    _description = "Gold Fixing Register"
    ...
    def _get_templates(self):
        templates = super(GoldFixingRegister, self)._get_templates()
        templates[
            'line_template'] = 'account_reports.line_template_general_ledger_report'
        return templates
    ...
    def _get_columns_name(self, options):
        return [
            ...
            {'name': _('Description')},
            {'name': _('Partner')},
            {'name': _('Qty IN'), 'class': 'number'},
            {'name': _('Qty OUT'), 'class': 'number'},
            {'name': _('Qty Balance'), 'class': 'number'},
            {'name': _('Amt. Rate'), 'class': 'number'},
            {'name': _('Amt. Value'), 'class': 'number'},
            {'name': _('Amt. Balance'), 'class': 'number'},
            {'name': _('Average'), 'class': 'number'}
        ]

    @api.model
    def _get_report_name(self):
        return _("Gold Fixing Register")
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if offset > 0:
            ...
            return self._get_gold_fixing_lines(options, line_id=line_id)

    @api.model
    def _get_gold_fixing_lines(self, options, line_id=None):
        ...
        unfold_all = options.get('unfold_all') or (
                    self._context.get('print_mode') and not options[
                'unfolded_lines'])
        ...
        accounts_results, taxes_results = self._do_query(options_list,
                                                         expanded_account=expanded_account)
        total_debit = total_credit = total_balance = 0.0
        total_pure_wt_in = total_pure_wt_out = total_gold_rate = total_gold_value = 0
        for account, periods_results in accounts_results:
            ...
            amount_currency = account_sum.get('amount_currency',
                                              0.0) + account_un_earn.get(
                'amount_currency', 0.0)
            pure_wt_in = account_sum.get('pure_wt_in',
                                         0.0) + account_un_earn.get(
                'pure_wt_in', 0.0)
            pure_wt_out = account_sum.get('pure_wt_out',
                                          0.0) + account_un_earn.get(
                'pure_wt_out', 0.0)
            gold_rate = account_sum.get('gold_rate', 0.0) + account_un_earn.get(
                'gold_rate', 0.0)
            gold_value = account_sum.get('gold_value',
                                         0.0) + account_un_earn.get(
                'gold_value', 0.0)
            debit = account_sum.get('debit', 0.0) + account_un_earn.get('debit',
                                                                        0.0)
            credit = account_sum.get('credit', 0.0) + account_un_earn.get(
                'credit', 0.0)
            balance = account_sum.get('balance', 0.0) + account_un_earn.get(
                'balance', 0.0)

            lines.append(
                self._get_account_title_line(options, account, amount_currency,
                                             debit, credit, balance, has_lines,
                                             pure_wt_in, pure_wt_out, gold_rate,
                                             gold_value))

            total_debit += debit
            total_pure_wt_in += pure_wt_in
            total_pure_wt_out += pure_wt_out
            total_gold_rate += gold_rate
            ...
            if has_lines and (unfold_all or is_unfolded):
                ...
                cumulated_balance = account_init_bal.get('balance',
                                                         0.0) + account_un_earn.get(
                    'balance', 0.0)

                lines.append(self._get_initial_balance_line(
                    options, account,
                    account_init_bal.get('pure_wt_in',
                                         0.0) + account_un_earn.get(
                        'pure_wt_in', 0.0),
                    account_init_bal.get('pure_wt_out',
                                         0.0) + account_un_earn.get(
                        'pure_wt_out', 0.0),
                    account_init_bal.get('gold_rate',
                                         0.0) + account_un_earn.get('gold_rate',
                                                                    0.0),
                    account_init_bal.get('gold_value',
                                         0.0) + account_un_earn.get(
                        'gold_value', 0.0),
                    account_init_bal.get('balance', 0.0) + account_un_earn.get(
                        'balance', 0.0),
                ))
                ...
                for aml in amls:
                    ...
                    lines.append(self._get_aml_line(options, account, aml,
                ...
                lines.append(self._get_account_total_line(
                    options, account,
                    account_sum.get('pure_wt_in', 0.0),
                    account_sum.get('pure_wt_out', 0.0),
                    account_sum.get('gold_rate', 0.0),
                    account_sum.get('gold_value', 0.0),
    ...
    def _load_more_lines(self, options, line_id, offset, load_more_remaining,
        ...
        amls_query, amls_params = self._get_query_amls(options,
                                                       expanded_account,
                                                       offset=offset,
        ...
        for aml in self._cr.dictfetchall():
            ...
            lines.append(self._get_aml_line(options, expanded_account, aml,
                                            balance_progress))
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(GoldFixingRegister, self)._get_options_domain(options)
    ...
    def _get_query_sums(self, options_list, expanded_account=None):
        ...
        domain = [('account_id', '=',
                   expanded_account.id)] if expanded_account else []
        ...
        for i, options_period in enumerate(options_list):
            ...
            queries.append('''
                SELECT
                    ...
                    COALESCE(SUM(account_move_line.pure_wt_in), 0.0)   AS pure_wt_in,
                    COALESCE(SUM(account_move_line.pure_wt_out), 0.0)   AS pure_wt_out,
                    COALESCE(AVG(account_move_line.gold_rate), 0.0)   AS gold_rate,
                    COALESCE(SUM(account_move_line.gold_value), 0.0)   AS gold_value,
        ...
        journal_options = self._get_options_journals(options)
        if not expanded_account and len(journal_options) == 1 and \
                journal_options[0]['type'] in ('sale', 'purchase'):
            for i, options_period in enumerate(options_list):
                tables, where_clause, where_params = self._query_get(
                    options_period)
    ...
    @api.model
    def _get_query_amls(self, options, expanded_account, offset=None,
                        limit=None):
        ...
        query = '''
            SELECT
                ...
                account_move_line.pure_wt_in AS pure_wt_in,
                account_move_line.pure_wt_out AS pure_wt_out,
                account_move_line.gold_rate AS gold_rate,
                account_move_line.gold_value   AS gold_value,
    ...
    def _do_query(self, options_list, expanded_account=None, fetch_lines=True):
        ...
        query, params = self._get_query_sums(options_list,
                                             expanded_account=expanded_account)
        ...
        for res in self._cr.dictfetchall():
            ...
            key = res['key']
            if key == 'sum':
                groupby_accounts.setdefault(res['groupby'], [{} for n in range(
                    len(options_list))])
                ...
                groupby_companies.setdefault(res['groupby'], [{} for n in range(
                    len(options_list))])
        ...
        if groupby_companies:
            ...
            for account in candidates_accounts:
                ...
                for i in range(len(options_list)):
                    ...
                    groupby_accounts.setdefault(account.id, [{} for i in range(
                        len(options_list))])
        ...
        if expanded_account:
            ...
            accounts = []
        accounts_results = [(account, groupby_accounts[account.id]) for account
                            in accounts]
    ...
    @api.model
    def format_gold_value(self, value):
        """
        Format Gold Value
        """
        if value:
            return '{0:.{1}f}'.format(value, 3)
        return ""

    @api.model
    def _get_account_title_line(self, options, account, amount_currency, debit,
                                credit, balance, has_lines, pure_wt_in,
                                pure_wt_out, gold_rate, gold_value):
        ...
        return {
            ...
            'title_hover': name,
            'columns': [
                {'name': self.format_gold_value(pure_wt_in), 'class': 'number',
                 'title': "Qty IN"},
                {'name': self.format_gold_value(pure_wt_out), 'class': 'number',
                 'title': "Qty OUT"},
                {'name': self.format_gold_value(0), 'class': 'number',
                 'title': "Qty Balance"},
                {'name': self.format_gold_value(gold_rate), 'class': 'number',
                 'title': "Amt. Rate"},
                {'name': self.format_gold_value(gold_value), 'class': 'number',
                 'title': "Amt. Value"},
                {'name': self.format_gold_value(0), 'class': 'number',
                 'title': "Amt. Balance"},
                {'name': self.format_gold_value(0), 'class': 'number',
            ...
            'unfolded': has_lines and 'account_%d' % account.id in options.get(
                'unfolded_lines') or unfold_all,
            'colspan': 4,
        }

    @api.model
    def _get_initial_balance_line(self, options, account, pure_wt_in,
                                  pure_wt_out, gold_rate, gold_value, balance):
        has_foreign_currency = account.currency_id and account.currency_id != account.company_id.currency_id or False
        return {
            ...
            'columns': [
                {'name': self.format_gold_value(pure_wt_in), 'class': 'number'},
                {'name': self.format_gold_value(pure_wt_out),
                 'class': 'number'},
                {'name': self.format_gold_value(0), 'class': 'number'},
                {'name': self.format_gold_value(gold_rate), 'class': 'number'},
                {'name': self.format_gold_value(gold_value), 'class': 'number'},
        ...
        return {
            ...
            'columns': [
                ...
                {'name': self._format_aml_name(aml['name'], aml['ref'],
                                               aml['move_name']),
                 'title': title, 'class': 'whitespace_print'},
                {'name': aml['partner_name'], 'title': aml['partner_name'],
                 'class': 'whitespace_print'},
                {'name': self.format_gold_value(aml['pure_wt_in']),
                 'class': 'number'},
                {'name': self.format_gold_value(aml['pure_wt_out']),
                 'class': 'number'},
                {'name': 0, 'class': 'number'},
                {'name': self.format_gold_value(aml['gold_rate']),
                 'class': 'number'},
                {'name': self.format_gold_value(aml['gold_value']),
                 'class': 'number'},
                {'name': 0, 'class': 'number', 'title': "001"},
            ],
            'level': 4,
        }

    @api.model
    def _get_load_more_line(self, options, account, offset, remaining,
                            progress):
        return {
    ...
    @api.model
    def _get_account_total_line(self, options, account, pure_wt_in, pure_wt_out,
                                gold_rate, gold_value, balance):
        return {
    ...
    @api.model
    def _get_total_line(self, options, pure_wt_in, pure_wt_out, gold_rate,
                        gold_value, balance):
        return {
    ...
    def _get_tax_declaration_lines(self, options, journal_type, taxes_results):
        ...
        tax_report_options = self.env[
            'account.generic.tax.report']._get_options()
        ...
        tax_report_lines = self.env['account.generic.tax.report'].with_context(
            force_company=journal.company_id.id)._get_lines(tax_report_options)

        for tax_line in tax_report_lines:
            if tax_line['id'] not in (
            'sale', 'purchase'):  # We want to exclude title lines here
